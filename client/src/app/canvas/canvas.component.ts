...
///////////
  ...
  findxy(res, e) {
    ...
    if(res == 'move') {
      if (this.flag) {
        ...
        if(this.shownPathsArr.length>100){
  ...
  updatePaths(){
    let observable = this.socket.addPaths(this.shownPathsArr);
    observable.subscribe(data =>{
      if(data['message'] === "Success"){
  ...
  postPaths(){
    let observable = this.socket.saveFirstPaths(this.shownPathsArr);
    observable.subscribe(data =>{
      if(data['message'] === "Success"){
